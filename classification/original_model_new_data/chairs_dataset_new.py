...
def load(dimension):
    ...
    for id, folder in enumerate(["chairs-data/renders/", "chairs-data/renders_bad/"]):
        ...
        for image_file in image_files:
            fileset.add(image_file.split("_")[0])
        print("Parsed all file names")
        
        for i in range(0, length):
            image_file = fileset.pop()
            for j in range(0, 7):
                image_file_view = image_file + "_" + str(j) + ".png"

                view = j
                img = cv2.imread(folder+image_file_view)
